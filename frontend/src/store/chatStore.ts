...
export const useChatStore = create<ChatState>((set, get) => ({
  ...
  openSession: async (sid) => {
    ...
    try {
      ...
      if (Array.isArray(data.messages) && data.messages.length > 0) {
        ...
        /* 会话只有辩论事件没有对话链（极端情况） */
        const msgs: ChatMessage[] = (data.debate_events as { id: number; event: string }[])
          .map((d) => {
            try {
              const ev = JSON.parse(d.event) as CouncilEvent
              return {
                id: `d${d.id}`,
                role: 'debate' as const,
                content: '',
                thinking: '',
                toolCalls: [],
                done: true,
  ...
  send: async (text, file, isRetry) => {
    ...
    const botMsg: ChatMessage = {
      ...
      userPrompt: text /* 2026-08-27 重试修复：记录原始用户输入——重试必须重发它，不是重发错误 */,
    }
    set((s) => ({
      streaming: true,
      runId: null,
    ...
    try {
      ...
      await fetchSSE(
        ...
        (ev) => {
          ...
          if (ev.type === 'delta') {
            ...
            if (m.content === '' && /^[\s，。、；：！？,.;:!?'"''（）()[\]{}…—~-]*$/.test(ev.text))
              return
            m.content += ev.text
            m.parts = [...(m.parts || []), { kind: 'text', text: ev.text }] /* P0-4 parts 保序 */
          } else if (ev.type === 'run') {
            set({ runId: ev.run_id })
          } else if (ev.type === 'thinking') {
            m.thinking += ev.text
            m.parts = [...(m.parts || []), { kind: 'thinking', text: ev.text }]
            ...
            if (ev.status === 'start' && !existing) {
              m.toolCalls.push(t)
              m.parts = [...(m.parts || []), { kind: 'tool', id: ev.id, name: ev.name, status: 'start', args: ev.args }]
            } else if ((ev.status === 'done' || ev.status === 'error') && existing) {
              existing.status = ev.status
              existing.result = ev.result
              m.parts = (m.parts || []).map((pr) =>
                pr.kind === 'tool' && pr.id === ev.id
                  ? { ...pr, status: ev.status, result: ev.result }
                  : pr,
              )
            }
            ...
            m.done = true
          } else if (ev.type === 'tool_only') {
            /* 2026-08-27：工具执行完成但模型未输出正文——提示而非报错（判断无墙，只让"没输出"可见） */
            m.toolOnly = true
            m.toolOnlyTools = ev.tools || []
          } else if (ev.type === 'truncated') {
            /* 2026-08-27 根治"输出一半无提示"：模型达单次输出上限被截断——内容有效，非错误 */
            m.truncated = true
            m.truncatedReason = ev.reason || 'output_length'
          } else if (ev.type === 'glint') {
            /* 闪光时刻（2026-08-20，拾光=拾到我们没发现的闪光）：收尾提炼的闪光 → 挂最后一条 assistant 消息 */
            m.glint = ev.text
          } else if (ev.type === 'system') {
            /* n=5 硬停止系统消息（2026-08-30 接地纠错循环）：harness 直发、非模型口吻——
               带「系统」徽标渲染；失败可见性由代码保证，模型无法吞掉 */
            m.systemNotes = [...(m.systemNotes || []), ev.text]
            m.parts = [...(m.parts || []), { kind: 'system', text: ev.text }]
      ...
      /* 星尘即时刷新（2026-08-21）：对话结束 30s 后派发记忆变更事件 → 星尘/路径面板自动 reload。
         30s 覆盖收尾提炼窗口（6 个提炼器串行 20-30s 才写入记忆——立即刷会读到旧数据）。 */
      window.setTimeout(() => {
        window.dispatchEvent(new Event('lumen-memory-changed'))
      }, 30000)
    }
  },
