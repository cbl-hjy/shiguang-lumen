...
const TOOL_ICON: Record<
  string,
  'search' | 'code' | 'image' | 'file' | 'book' | 'download' | 'brain' | 'user' | 'trash' | 'wrench'
> = {
  web_search: 'search',
  python_sandbox: 'code',
  ocr_image: 'image',
  ...
  return (
    <div
      className={`inline-flex items-center gap-1.5 mb-1.5 mr-2 px-2.5 py-1 rounded-lg border text-[12px] ${
        failed ? 'border-error/50 bg-error/10 text-error/90' : 'border-hairline bg-surface'
      } ${done || failed ? 'animate-tool-done' : ''}`}
      title={failed && call.result ? `工具失败：${call.result}` : undefined}
    >
      <Icon
        name={TOOL_ICON[call.name] ?? 'wrench'}
        size={13}
        className={failed ? '' : 'text-ink-muted'}
      />
      ...
      {failed ? (
        <span className="text-error/90">
          {call.result ? `⚠ ${call.result.slice(0, 36)}` : '⚠ 失败'}
        </span>
      ) : done ? (
