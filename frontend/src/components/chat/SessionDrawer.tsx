...
export default function SessionDrawer({
  ...
  return (
    <div
      className="fixed inset-0 z-40 bg-black/50 flex items-start justify-start animate-toast-in"
      onClick={onClose}
    >
      {/* 2026-08-28 半屏浮层（UX 优化 C5，对标 LobeChat"默认隐藏+按需唤出"）：
          min(85vw, 320px) 不遮挡主内容；遮罩变轻 */}
      <div
        className="w-[min(85vw,320px)] h-full bg-elevated border-r border-hairline flex flex-col drawer-left-in"
        style={{ paddingTop: 'env(safe-area-inset-top)' }}
        onClick={(e) => e.stopPropagation()}
        role="dialog"
        aria-label="夜谈记录"
      >
        <div className="flex items-center justify-between px-4 min-h-12 border-b border-hairline shrink-0">
            ...
            sessions.map((s) => {
              ...
              return (
                <div
                  ...
                  onClick={() => onOpen(s.id)}
                >
                  <div className="text-[13px] text-ink truncate pr-6">{s.title || '新夜谈'}</div>
