...
type Tab = 'memory' | 'reflection' | 'skill' | 'kb' | 'health'

/* 数组兜底（2026-08-29 白屏事故修复）：后端字段缺失/为 null/非数组 → 空数组。
   `.length of undefined` 是 render 期异常，React 18 会卸载整棵树 → 用户只剩背景。 */
...
function ChangeRow({ c }: { c: ChangeRecord }) {
  ...
  const label = c.action.includes('remember')
    ? '新增'
    : c.action.includes('forget')
      ? '熄星'
      : c.action.includes('edit')
        ? '修正'
        : '增强'
  ...
  return (
    <div className="flex items-start gap-2 py-1.5 border-b border-hairline/50 last:border-0">
      <span
        className={`shrink-0 mt-0.5 text-[10px] px-1.5 py-0.5 rounded ${actionColor[c.action] ?? 'text-ink-dim'} bg-elevated`}
      >
        {label}
      </span>
...
export default function MemoryPanel() {
  ...
  /* 三个数组字段统一兜底：/api/memory 若缺字段（历史契约不一致）不崩渲染 */
  ...
  /* 星尘即时刷新（2026-08-21）：对话结束 30s 事件 → reload（不用 F5 看新记忆/画像/反思） */
  useEffect(() => {
    const onChanged = () => reload()
    window.addEventListener('lumen-memory-changed', onChanged)
    return () => window.removeEventListener('lumen-memory-changed', onChanged)
  }, [reload])
  ...
  const tabs: { key: Tab; label: string; count: number }[] = [
    { key: 'memory', label: '星尘', count: entries.length },
    { key: 'health', label: '健康', count: 0 },
    { key: 'reflection', label: '反思', count: reflections.length },
    { key: 'skill', label: '技能', count: skills.length },
    { key: 'kb', label: '知识库', count: 0 },
  ]

  return (
    <div className="h-full flex flex-col min-h-0">
      <h2 className="text-[13px] font-medium text-ink px-4 pt-4 pb-2">星尘</h2>
      {/* tab 切换 */}
      <div className="flex px-4 pb-2 gap-1">
        ...
        {tab === 'memory' && data && entries.length > 1 && (
          <button
            ...
            title={
              sortDesc ? '当前：最新在上，点击切为最早在上' : '当前：最早在上，点击切为最新在上'
            }
            aria-label="切换星尘排序"
          >
            {sortDesc ? '最新 ↓' : '最早 ↑'}
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-4 flex flex-col gap-2">
        {tab === 'kb' ? (
          ...
          /* 记忆健康（Phase1 M-4 治理入口）：活跃度 + 将要休眠的记忆——讲人话不堆术语 */
          <MemoryHealth entriesCount={entries.length} />
        ) : data === null ? (
          /* 加载骨架 */
          <>
            ...
            {[0, 1, 2].map((i) => (
              <div
                key={i}
                className="rounded-lg border border-hairline bg-surface p-2.5 animate-pulse"
              >
          ...
          <>
            {tab === 'memory' && (
              <>
                ...
                <DetailModal title="画像" open={profileOpen} onClose={() => setProfileOpen(false)}>
                  {data.profile ?? '(暂无画像)'}
                </DetailModal>
