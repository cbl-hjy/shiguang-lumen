      ...
      >
        <div className="flex items-center justify-between px-4 py-3 border-b border-hairline shrink-0">
          ...
          <button
            onClick={onClose}
            aria-label="关闭"
            className="p-1 rounded hover:bg-surface text-ink-dim hover:text-ink/85 transition-colors"
          >
            <Icon name="x" size={16} />
          </button>
        </div>
        <div className="flex-1 min-h-0 overflow-y-auto px-4 py-3 text-[14px] leading-relaxed text-ink/90 whitespace-pre-wrap">
          {children}
        </div>
        {footer && (
          <div className="px-4 py-2.5 border-t border-hairline flex justify-end gap-2 shrink-0">
            {footer}
          </div>
        )}
      </div>
