...
export default function NotificationBanner() {
  ...
  useEffect(() => {
    const poll = async () => {
      try {
        const items = await fetchNotifications()
        const fresh = items.filter((i) => !i.is_read && !seenRef.current.has(i.id))
        if (fresh.length) {
  ...
  return (
    <div
      className="fixed top-14 right-4 z-40 max-w-sm animate-msg-in"
      role="status"
      aria-live="polite"
    >
      <div className="relative rounded-xl bg-elevated border border-hairline shadow-2xl overflow-hidden glow-primary">
        ...
        <div className="flex items-start gap-3 pl-4 pr-3 py-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="text-[12px] font-medium text-ink mb-0.5">拾光记得你</div>
            <p className="text-[13px] leading-relaxed text-ink-muted">{current.content}</p>
          </div>
          <button
            onClick={dismiss}
