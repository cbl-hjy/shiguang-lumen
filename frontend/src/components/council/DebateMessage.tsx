...
function TurnCard({
  ...
  return (
    <div
      className={
        open
          ? 'border-l-2 border-[rgba(62,201,176,0.35)] pl-3'
...
function ReportCard({ ev }: { ev: Extract<CouncilEvent, { type: 'report' }> }) {
  ...
  return (
    <div className="border-l-2 border-primary/40 pl-3">
      ...
      {errText && state === 'err' && (
        <div className="mt-1.5 text-[11px] text-[rgba(230,120,90,0.9)]">{errText}</div>
      )}
      {detail && (
        <div className="mt-2 space-y-1 text-[12px] leading-6 text-ink-muted">
          ...
          {ev.unanswered.length > 0 && (
            <div className="text-[rgba(230,180,120,0.7)]">未：{ev.unanswered.join('；')}</div>
          )}
        </div>
      )}
    </div>
...
export default function DebateMessage({ ev, msgId }: { ev: CouncilEvent; msgId: string }) {
  ...
  if (ev.type === 'turn') {
    /* 是否是本场辩论第一条星宿发言（默认展开） */
    const firstTurnId = messages.find(
      (m) => m.role === 'debate' && m.debateEvent?.type === 'turn',
    )?.id
