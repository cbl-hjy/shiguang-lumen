...
export async function startDebate(
  ...
  await fetchSSE('/api/council/debate', body, onEvent as unknown as (ev: SSEEvent) => void, {
    signal,
  })
}
...
export async function updateSageMeta(
  sageId: string,
  name?: string,
  stance?: string,
): Promise<void> {
...
/* 未完成蒸馏任务列表（断点续传入口） */
export async function fetchDistillJobs(): Promise<
  Array<{
    job_id: string
    sage_id: string
    book_title: string
