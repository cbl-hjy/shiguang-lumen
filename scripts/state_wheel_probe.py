...
def chat(msg: str, sid: str | None = None) -> tuple[bool, str]:
    ...
    r = subprocess.run(
        ...
        check=False,  # 显式：curl 瞬时失败返回非 0，由下方可见化 + 调用方按 stdout 判定
    )
    if r.returncode != 0:
        # 2026-09-02 修 PLW1510：不是只写 check=False 满足规则，而是让失败可见
        print(f"[probe] chat 调用失败 rc={r.returncode}: {r.stderr[:200]}", file=sys.stderr)
...
def new_sid() -> str:
    r = subprocess.run(["curl", "-s", "-X", "POST", f"{BASE}/api/session/new"] + curl_auth() + ["--max-time", "10"],
                       ...
                       check=False)  # 显式 check + 失败可见化（修 PLW1510，不是只满足规则）
    if r.returncode != 0:
        print(f"[probe] 创建会话失败 rc={r.returncode}: {r.stderr[:200]}", file=sys.stderr)
...
def stage_l1():
    ...
    with open(T.__file__, encoding="utf-8") as tf, open(M.__file__, encoding="utf-8") as mf:
        t_txt, m_txt = tf.read(), mf.read()
    check("tutor 注入路径", "inject_state" in t_txt)
    check("main 快照路径", "snapshot_to_last_session" in m_txt)
...
def stage_l3():
    ...
    with open(T.__file__, encoding="utf-8") as tf, open(W.__file__, encoding="utf-8") as wf:
        t_src, w_src = tf.read(), wf.read()
    ...
    # 注入文本无结论句
    with open(ROOT / "app" / "memory" / "state.py", encoding="utf-8") as sf:
        s_src = sf.read()
