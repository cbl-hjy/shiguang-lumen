...
# 索引文件 → 应列出所在目录下的 md 文件（personal-doc 已迁出项目：个人文档域 2026-08-15 起独立于 <personal-docs>\，见 PROJECT-LAYOUT B 节）
INDEXES = {
...
def check_docs_purity() -> list[str]:
    """项目内零个人文档文件（用户红线 2026-08-15：个人文档完全迁出项目 → 个人文档目录 <personal-docs>\\）。
    个人文档类文件（文件名含 personal-doc/个人文档）一旦出现在项目顶层或 docs/ 即报错。"""
    import re
    issues = []
    for base, label in ((ROOT, "顶层"), (DOCS, "docs/")):
        ...
        for p in base.iterdir():
            if p.is_dir() or p.suffix.lower() not in (".md", ".docx", ".pdf"):
                continue
            if re.search(r"personal-doc|个人文档", p.name, re.IGNORECASE):
                issues.append(f"项目内发现个人文档文件: {p.relative_to(ROOT)}（应进个人文档目录 <personal-docs>\\，项目内零个人文档文件）")
    return issues
