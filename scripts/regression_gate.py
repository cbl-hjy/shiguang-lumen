...
def chat(msg: str, sid: str | None = None) -> tuple[bool, str]:
    ...
    r = subprocess.run(
        ...
        check=False, capture_output=True, text=True, encoding="utf-8", errors="ignore", timeout=100,
    )
    ...
    try:
        r = subprocess.run(
            ...
            + ["-d", json.dumps({"old": "回归门测试条目 GATE_1", "new": "回归门测试条目 GATE_1（端点探针）"},
                                ensure_ascii=False),
             "--max-time", "30"],
            check=False, capture_output=True, text=True, encoding="utf-8", errors="ignore", timeout=40,
        )
