...
def curl_json(args: list[str]) -> dict:
    r = subprocess.run(["curl", "-s"] + args, capture_output=True, text=True,
                       ...
                       check=False)  # 失败由调用方按 stdout 判定（curl_json 返回 {}）
    try:
...
def main():
    ...
    _g = subprocess.run(["git", "status", "--short"], capture_output=True, text=True,
                        encoding="utf-8", cwd=str(ROOT), check=False)  # returncode 在下一行判定
    _gs = _g.stdout.strip()
    ...
    _head = subprocess.run(["git", "rev-parse", "--verify", "HEAD"], capture_output=True,
                           text=True, encoding="utf-8", cwd=str(ROOT), check=False)
    ...
    if sid.get("session_id") and "--full" in sys.argv:
        ...
        r1 = subprocess.run(["curl", "-s", "-N", "-X", "POST", f"{BASE}/api/chat", "-H", "Content-Type: application/json",
                             ...
                             "--max-time", "150"], capture_output=True, text=True, encoding="utf-8", errors="ignore",
    ...
    r_pytest = subprocess.run(
        ...
        timeout=900, cwd=str(ROOT), check=False,
    )
    ...
    dh_ok = r_dh.returncode in (0, 1)  # 黄可接受（如个人文档线历史豁免），红=真问题
    ...
    if not _is_9000_up():
        ...
        with open(str(ROOT / "data-experiment" / "e2e.log"), "w", encoding="utf-8") as logf:
            subprocess.Popen(
    ...
    e2e_ok = _is_9000_up()
    e2e_ev = []
    # 发布版：e2e_journey.py 不在公开仓 → 缺则跳过该步
    if e2e_ok and (ROOT / "scripts" / "e2e_journey.py").exists():
