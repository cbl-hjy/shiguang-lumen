...
def chat(msg: str, sid: str, tag: str) -> dict:
    ...
    r = subprocess.run(
        ...
        check=False,  # 失败按 stdout 判定（done 标志）；非零退出打 stderr
    )
    if r.returncode != 0:
        print(f"[praise] chat 调用失败 rc={r.returncode}: {r.stderr[:120]}", file=__import__('sys').stderr)
    out = r.stdout
    tools = {}
        ...
        for line in out.splitlines():
            if line.startswith("data:") and '"type": "delta"' in line:
                try:
                    ...
                    print(f"[praise] 跳过坏 delta 行: {_je}", file=__import__('sys').stderr)
...
def main():
    ...
    if REFLECTIONS.exists():
        with open(REFLECTIONS, encoding="utf-8") as rf:
            before_r = sum(1 for _ in rf)
    if SKILLS.exists():
        with open(SKILLS, encoding="utf-8") as sf:
            before_s = sum(1 for _ in sf)

    r = subprocess.run(["curl", "-s", "-X", "POST", f"{BASE}/api/session/new", "--max-time", "10"],
                       capture_output=True, text=True, encoding="utf-8", check=False)
    if r.returncode != 0:
        print(f"[praise] 建会话失败 rc={r.returncode}: {r.stderr[:120]}", file=__import__('sys').stderr)
    ...
    if REFLECTIONS.exists():
        with open(REFLECTIONS, encoding="utf-8") as rf:
            after_r = sum(1 for _ in rf)
    if SKILLS.exists():
        with open(SKILLS, encoding="utf-8") as sf:
            after_s = sum(1 for _ in sf)
    print("\n=== 一号病菌验证 ===", flush=True)
