...
def analyze_image(image_path: str, question: str = "") -> str:
    ...
    if not api_key:
        return tool_err(
            "看图", "未配置 ZHIPU_API_KEY（.env 加一行）", "可改用 ocr_image 提取图片文字"
        )
    p = Path(image_path)
    if not p.exists():
    ...
    if img is None:
        try:
            if p.stat().st_size > MAX_IMG_BYTES:
                return arg_error(
                    "看图", "图片超过 5M 限制", "压缩图片后重试；需要文字可改用 ocr_image"
                )
    ...
    try:
        ...
        if "429" in str(e) or "Limit" in type(e).__name__ or "1305" in str(e):
            try:
                ...
                return tool_err(
                    "看图",
                    f"智谱视觉分析限流（双模型均被限）: {str(e2)[:60]}",
                    "稍后重试，或改用 ocr_image 提取图片文字",
                )
        ...
        return tool_err(
            "看图", f"智谱视觉分析失败: {str(e)[:80]}", "稍后重试，或改用 ocr_image 提取图片文字"
        )
