...
# 记忆文件路径（2026-08-28 P0 多用户）：
# - legacy 常量保留（未登录/未迁移场景兜底）
# - 用户场景统一走 memory_dir(uid)：data/users/<uid>/memory/
MEMORY_FILE = DATA_DIR / "memory" / "user_memory.md"
PROFILE_FILE = DATA_DIR / "memory" / "profile.md"


def memory_dir(uid: str | None = None) -> Path:
    """当前用户记忆目录（contextvar 兜底）：uid 显式 > contextvar > 全局 legacy"""
    if not uid:
    ...
    if uid:
        ...
        return get_user_data_dir(uid) / "memory"
...
# 2026-08-28 Phase1 M-2（FadeMem arXiv:2601.18642）：按类别差异化半衰期——
# 越"身份级"的记忆忘得越慢（偏好/目标/关系），越"事件级"的越快（困惑/笔记）。
# 半衰期 τ（天）：衰减到保留分 1/e 所需天数；用于 rec = e^(-days/(S×τ)) 与冷记忆降级判定。
CAT_TAU = {
    "偏好": 120,  # 身份级：近 4 个月（用户的喜欢/习惯）
    "目标": 120,  # 身份级：目标长存
    "关系": 120,  # 身份级：关系是长期资产
    "进度": 60,  # 成长级：2 个月
    "学习记录": 60,  # 成长级
    "错误记录": 45,  # 成长级：错误要记但会过时
    "反思": 45,  # 沉淀级
    "闪光": 30,  # 沉淀级：闪光观察 1 个月后弱化
    "笔记": 30,  # 事件级
    "困惑": 21,  # 事件级：具体困惑很快解决/过时
}


def cat_tau(category: str) -> int:
    """类别 → 半衰期（天）。词汇表外类别兜底 45 天。"""
    return CAT_TAU.get(category, 45)
