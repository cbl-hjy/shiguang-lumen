...
_STATE_OVERRIDE: Path | None = None  # 测试注入点（2026-08-28：存量测试从模块常量注入迁移）


def STATE_FILE() -> Path:
    return _STATE_OVERRIDE if _STATE_OVERRIDE is not None else memory_dir() / "state.json"
    ...
    for dim, val in updates.items():
        ...
        if isinstance(val, str) and val.lstrip().startswith("{"):
            try:
                ...
                print(f"[memory/state] 静默异常已可见化: {_e}", flush=True)
...
def update_relation(
    depth: str = "", last_topic: str = "", tone: str = "", evidence: str = ""
) -> str:
...
def inject_state() -> str:
    ...
    if cur:
        ...
        if dims:
            parts.append(
                f"本次会话状态（更新于 {state.get('updated_at', '?')}）：{'；'.join(dims)}"
            )
    # 关系轨（慢变，有才注入，≤50 字）
...
def _dim_cn(dim: str) -> str:
    return {"emotion": "情绪", "blocker": "卡点", "pace": "节奏", "willingness": "意愿"}.get(
        dim, dim
    )
