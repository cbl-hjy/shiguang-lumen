...
class EvolutionEditRequest(BaseModel):
...
@router.post("/api/evolution/edit")
async def evolution_edit(req: EvolutionEditRequest):
    """M5 人可改：编辑反思/技能条目（文件替换 + 向量重灌）"""
    from app.agent.evolution import edit_item, reindex_evolution

    msg = edit_item(req.kind, req.old, req.new)
    if "已" in msg:
        await reindex_evolution()
...
def sessions_list(request: Request):
    ...
    return {"sessions": list_sessions(getattr(request.state, "user_id", None))}


@router.delete("/api/session/{sid}")
def session_delete(sid: str, request: Request):
    ...
    ok = delete_session(sid, getattr(request.state, "user_id", None))
    if not ok:
...
def session_new(request: Request):
    ...
    return {"session_id": new_session(getattr(request.state, "user_id", None))}
...
def get_session(sid: str, request: Request):
    ...
    raw = load_messages_json(sid, _uid)
    out = []
    if raw:
        try:
            msgs = ModelMessagesTypeAdapter.validate_json(raw)
        except Exception as e:
            return {"messages": [], "debate_events": load_debate_events(sid, _uid), "error": str(e)}
        for m in msgs:
    ...
    # A2 双通道补全（2026-08-21 A3 双保险）：messages 表最后 user 行若未被链覆盖（异常中断轮次，
    # 如 request_limit/超时——旧实现 _finalize 落空链导致刷新丢最新对话）→ 追加"（回复中断）"占位。
    # 刷新永不"对话消失"，最坏显示"我说了什么 + 中断标记"。
    try:
        from app.db.sessions import db_session
        ...
        if lu:
            last_chain_user = next(
                (m["content"] for m in reversed(out) if m["role"] == "user"), None
            )
            if last_chain_user != lu["content"]:
                out.append(
                    {
                        "role": "user",
                        "content": (lu["content"] or "") + "（回复中断）",
                        "thinking": "",
                        "toolCalls": [],
                        "done": False,
                        "interrupted": True,
        ...
        print(f"[routers/panels] 静默异常已可见化: {_e}", flush=True)
    return {"messages": out, "debate_events": load_debate_events(sid, _uid)}
...
@router.get("/api/session/{sid}/summary")
def session_summary(sid: str, request: Request):
    """历史纪要（Phase 2 C-1，2026-08-29）：长会话早期历史压缩成的四字段纪要——
    UI「它记得我」历史纪要入口数据源。无纪要/未压缩 → 返回 None（前端隐藏卡片）。"""
    from app.agent.session_compress import parse_stored
    from app.db.sessions import get_summary

    raw = get_summary(sid, getattr(request.state, "user_id", None))
    meta = parse_stored(raw) if raw else None
    if not meta:
        return {"summary": None}
    return {
        "summary": {
            ...
            "covered_rounds": int(meta.get("covered_rounds", 0) or 0),
