...
async def _run_subtask(agent: Agent, task: str) -> str:
    ...
    try:
        ...
        print(f"[delegation] 子任务 token 台账写入失败（不影响子任务）: {e}", flush=True)
...
async def deleg_study(
    ...
    try:
        results = await asyncio.gather(
            *[_run_subtask(a, t) for a, t in zip(agents, tasks, strict=True)],
        ...
        if tracker:
            for name in tasks:
                try:
                    ...
                    print(f"[agent/delegation] 静默异常已可见化: {_e}", flush=True)
        raise

    parts: list[str] = []
    for name, res in zip(tasks, results, strict=True):
