...
from app.memory.vector import BGE_M3_PATH, aembed
...
def recent_pair(limit: int = 1) -> str:
    """注入用：最近 1 条失败教训 + 最近 1 条成功讲法（Reflexion+Voyager 注入层合并）。
    护栏=提供信息：标签分开，让模型对两种声音用不同判断层——
    教训按它改进（该避的坑），讲法可参考不必套用（成功路径，未必适用当前场景）。
    Reflexion 为"同一任务重试"设计，学习对话不重试——纯失败注入是论文迁移的语境错位。"""
    parts = []
    # 失败教训：最近反思
    if REFLECTIONS_FILE.exists():
        blocks = [
            b.strip()
            for b in REFLECTIONS_FILE.read_text(encoding="utf-8").split("## ")
            if b.strip()
        ]
        if blocks:
            parts.append("【失败教训】（遇到类似情况按它改进）\n" + blocks[-1][:600])
    # 成功讲法：最近技能（描述+方法截断；需完整时模型自己 search_skills）
    if SKILLS_FILE.exists():
        blocks = [
            b.strip()
            for b in SKILLS_FILE.read_text(encoding="utf-8").split("## ")
            if b.strip()
        ]
        if blocks:
            parts.append("【成功讲法】（可参考，不必套用；需完整讲法时用 search_skills）\n" + blocks[-1][:400])
    if not parts:
        return "(暂无教训与讲法)"
    return "\n\n".join(parts)
