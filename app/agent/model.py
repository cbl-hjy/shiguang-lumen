...
from app.config import DEEPSEEK_API_KEY, DEEPSEEK_BASE_URL, DEEPSEEK_MODEL, ENV
from app.config import model_tier as _config_model_tier  # C-2：档位表在 config（单一真相源）

# ---------- 模型版本 pin（集中一处：换模型只改 .env 的 DEEPSEEK_MODEL，回归门读它做版本指纹）----------
PIN_MODEL = DEEPSEEK_MODEL

# ---------- 模型档位（Phase 2 C-2，2026-08-29：注入预算显式化）----------
# 定义见 app/config.py MODEL_TIERS（单一真相源）；本模块转发给 agent 层引用点


def model_tier() -> dict:
    """当前模型档位（按 DEEPSEEK_MODEL 名匹配；未知模型回退从严档）。"""
    return _config_model_tier()
...
def circuit_record(ok: bool):
    ...
    if ok:
        ...
        if _CIRCUIT["fails"] >= CIRCUIT_MAX_FAILS:
            ...
            print(
                f"[circuit] 连续 {CIRCUIT_MAX_FAILS} 次失败，熔断冷却 {CIRCUIT_COOL_SECONDS}s",
                flush=True,
            )
