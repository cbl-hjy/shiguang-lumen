...
class DebateRequest(BaseModel):
    ...
    sage_ids: list[str] | None = (
        None  # 可省略：未传时按 mode 预设解析（默认模式=固定星宿，自由定义未上）
    )
...
class DistillRequest(BaseModel):
    text: str = Field(
        ...
        description="书本文本（新建任务必填；续传可空——后端读存盘 raw）",
    )
    sage_id: str = Field(pattern=r"^[a-z0-9_-]{2,30}$")
    title: str = Field(default="", max_length=60)
    job_id: str | None = Field(
        default=None, max_length=40, description="断点续传任务 id（续传时带，可不传文本）"
    )
    ...
    for mid, m in modes.MODES.items():
        ...
        for sid in m["sage_ids"]:
            try:
                ...
                print(f"[council/api] 静默异常已可见化: {_e}", flush=True)
        out.append(
            {
                "id": mid,
                "name": m["name"],
...
async def council_debate(req: DebateRequest):
    ...
    if not sage_ids:
        ...
        raise HTTPException(
            status_code=422,
            detail=f"模式「{mode['name'] if mode else req.mode}」当前书库不支持（需扩充同领域书籍）",
        )
    try:
    ...
    async def gen():
        ...
        async for ev in engine.stream_debate(
            req.question, sage_ids, max_rounds, debate_id, req.mode
        ):
...
async def council_distill(req: DistillRequest):
    ...
    async def gen():
        try:
            async for ev in distill_core.stream_distill(
                req.text, req.sage_id, req.title, req.job_id
            ):
                if ev["type"] == "done":
                    card = ev["sage"]
                    (DATA_DIR / "data" / "sages" / f"{req.sage_id}.json").write_text(
                        __import__("json").dumps(card, ensure_ascii=False, indent=2),
                        encoding="utf-8",
                    )
