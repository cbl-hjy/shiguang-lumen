...
SAGE_LIMITS = UsageLimits(
    request_limit=12, total_tokens_limit=24000
)  # 2026-08-21 二次加宽：request 8→12、total 16000→24000（培根树落库后树证据更长，星宿翻树工具循环仍偶发超限——verification-deep-20260821 笛卡尔第1/2轮失败实证）
...
def _sage_prompt(card: SageCard, round_no: int, budget: int, has_tree: bool = False) -> str:
    hint = (
        "第一轮，充分立论。" if round_no == 1 else "前面已经说过的不要复述，只补充新观点（增量）。"
    )
...
def _build_sage_agent(
    card: SageCard, round_no: int, budget: int, counter: dict | None = None
) -> Agent:
    ...
    if has_tree:
        return Agent(
            get_model(),
            system_prompt=_sage_prompt(card, round_no, budget, True),
...
async def _speak(
    card: SageCard, question: str, history: list[DebateTurn], round_no: int
) -> DebateTurn | None:
    ...
    try:
        ...
        try:
            ...
            print(f"[council/engine] 静默异常已可见化: {_e}", flush=True)
        return None
    ...
    return DebateTurn(
        ...
        speech=speech,
        words=len(speech),
        tool_calls=counter.get("calls", 0),
    )
...
async def _judge(
    question: str, turns: list[DebateTurn], prev: ModeratorVerdict | None, mode: str = "cross"
) -> ModeratorVerdict:
...
async def _final_report(
    question: str, turns: list[DebateTurn], mode: str = "cross"
) -> DebateReport:
    ...
    return DebateReport(
        actionable=["（综合报告生成失败——请直接阅读上方原始发言）"], unanswered=[question]
    )
...
def _announce(record: DebateRecord, sages: list[SageCard]) -> None:
    ...
    est_out = (
        sum(WORD_BUDGETS.get(r, 100) for r in range(1, record.max_rounds + 1)) * len(sages) * 1.5
    )
    print("📋 会议预算告知")
    print(f"  星宿：{len(sages)} 位（{' × '.join(s.name for s in sages)}）")
    print(f"  轮数上限：{record.max_rounds}（观点穷尽即止）｜模式：{record.mode}")
    print(
        f"  预计 token：输入 ~{est_in // 1000}K + 输出 ~{int(est_out) // 1000}K ≈ {(est_in + int(est_out)) // 1000}K"
    )
    print(
        f"  预计成本：≈ ¥{(est_in + int(est_out)) * 0.000002:.3f}-{((est_in + int(est_out)) * 2) * 0.000004:.3f}（可随时中止）\n"
    )
    ...
    for round_no in range(1, max_rounds + 1):
        ...
        for card in sages:
            ...
            if turn is None:
                ...
                yield {
                    "type": "error",
                    "message": f"{card.name} 第{round_no}轮发言失败（超时或异常），已跳过",
                    "debate_id": did,
                }
                continue
...
async def run_debate(
    question: str, sage_ids: list[str], max_rounds: int = MAX_ROUNDS_DEFAULT
) -> DebateRecord:
...
async def _main() -> None:
    ...
    record = await run_debate(
        args.question, [s.strip() for s in args.sages.split(",") if s.strip()], args.max_rounds
    )
    if record.report:
        _print_report(record.report)
