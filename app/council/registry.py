...
def load_index() -> dict:
    if INDEX_FILE.exists():
        try:
            ...
            print(f"[council/registry] 静默异常已可见化: {_e}", flush=True)
    ...
    for p in sorted(SAGES_DIR.glob("*.json")):
        ...
        try:
            card = json.loads(p.read_text(encoding="utf-8"))
                ...
                print(f'[registry] 跳过异常项: {_e}', file=__import__('sys').stderr)
                continue
        ...
        entries.append(
            {
                ...
                "confirmed": card.get("audit", {}).get(
                    "user_confirmed", old.get("confirmed", False)
                ),
