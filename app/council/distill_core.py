...
LLM_LIMITS = UsageLimits(
    request_limit=5, total_tokens_limit=12000
)  # 三重验证字段使输出变长（9255 实测超 8000）
...
def split_parts(body: str) -> list[tuple[str, str]]:
    ...
    marks = [
        ...
        for m in re.finditer(
            r"^(?:PART [IVX]+|第[一二三四五六七八九十]+[章部节]|CHAPTER \w+|Chapter \d+|Part \w+|Part \d+)\s*$",
            body,
            re.M,
        )
    ]
...
async def _llm_json(prompt: str, content: str, label: str) -> dict:
    ...
    from app.agent.model import circuit_record, get_fallback_model, should_fallback
    ...
    for i, model in enumerate(models):
        ...
        try:
            async with asyncio.timeout(
                120
            ):  # 硬护栏：单次调用 120s 上限（主服务 240s 总护栏的脚本版）
...
async def _llm_json_retry(
    prompt: str, content: str, label: str, retries: int = 1
) -> dict | Exception:
...
def save_new_job(
    job_id: str, sage_id: str, book_title: str, raw_text: str, total_parts: int
) -> None:
        ...
        for line in sf.read_text(encoding="utf-8").splitlines():
            try:
                summaries.append(json.loads(line))
                    ...
                    print(f'[distill] 跳过异常项: {_e}', file=__import__('sys').stderr)
                    continue
...
def delete_job(job_id: str) -> None:
    ...
    清理失败只记录不抛：job 目录留着最多占点空间，让 delete_job 抛异常
    却会打断"蒸馏完成 → 入库"这条主链路。
    """
    from app.utils.safe_cleanup import try_rmtree

    try_rmtree(_job_dir(job_id), context="council/distill_core")
...
async def stream_distill(
    raw_text: str, sage_id: str, book_title: str = "", job_id: str | None = None
) -> dict:
        ...
        for i in range(0, len(chunks), BATCH):
            ...
            chunk_results.extend(
                await asyncio.gather(
                    *[
                        _llm_json_retry(CHUNK_PROMPT, c, f"{pname}-c{i + j}")
    ...
    yield {
        "type": "stage",
        "stage": "完成",
        "detail": f"{sage['audit']['quote_verified']}/{len(sage.get('core_claims', []))} 条引用逐字命中",
    }
...
async def _distill_impl(raw_text: str, sage_id: str, book_title: str, out_dir: Path) -> dict:
    ...
    for pname, ptext in parts:
        ...
        cs = await _llm_json(
            CHAPTER_PROMPT, json.dumps(ok, ensure_ascii=False)[:12000], f"{pname}-chapter"
        )
        chapter_summaries.append({"part": pname, **cs})
    sage = await _llm_json(
        SAGE_PROMPT, json.dumps(chapter_summaries, ensure_ascii=False), "sage-card"
    )
    sage.update(
        {
            "id": sage_id,
    ...
    no_v2 = [
        c.get("title", "?") for c in sage.get("core_claims", []) if not c.get("novel_question")
    ]
    ...
    (out_dir / f"{sage_id}.json").write_text(
        json.dumps(sage, ensure_ascii=False, indent=2), encoding="utf-8"
    )
    return sage
